-module(erldigest_encoder_tests).
...
decode_simple_challenge_test() ->
  Challenge = <<"Digest realm=\"http-auth@example.org\", ",
                        ...
                        "algorithm=SHA256, ",
                        "qop=\"auth,auth-int\"">>,
  {ok, Result} = erldigest_encoder:decode_challenge_headers(Challenge),
...
encode_challenge_response_test() ->
  Response = #{username => <<"Mufasa">>,
               ...
               qop => <<"auth">>,
               nc => <<"00000001">>},
  {ok, Result} = erldigest_encoder:encode_response_headers(Response),
